class Plot {
    ...
    ArrayList<GCodeCommand> gcodeQueueCopy = new ArrayList<GCodeCommand>();
    ...
    protected void queueGcode(String cmd, int pathIdx, int lineIdx) {
        globalQueueIndex++;
        gcodeQueue.add(new GCodeCommand(cmd, pathIdx, lineIdx, globalQueueIndex));
    }
    ...
    public void plot() {
        ...
        generatePathGcode();
        gcodeQueueCopy = new ArrayList<>(gcodeQueue);
    }
            ...
            public void run() {
                while (plotting && !paused && hasMoreGcode()) {
                    try {
                        ...
                        GCodeCommand cmd = getNextGcode();
                        if (cmd.command != null) {
                            print(cmd.command);
                            println(cmd.queueIndex);
                            myTextarea.setText(" " + cmd.command);
